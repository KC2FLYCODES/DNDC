  ...
  const renderStories = () => (
    <div>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: '#718096' }}>Real stories from real residents who found success through DNDC programs</p>
      </div>
      ...
      {selectedStory ? (
        <div style={{ background: 'white', padding: '2rem', borderRadius: '12px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
          <button
            ...
            style={{ background: 'transparent', border: 'none', color: '#667eea', cursor: 'pointer', marginBottom: '1rem' }}
          >
            ← Back to stories
          </button>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            {selectedStory.program_name && (
              <span style={{ marginLeft: '1rem', background: '#e6f2ff', color: '#667eea', padding: '0.25rem 0.75rem', borderRadius: '20px', fontSize: '0.85rem' }}>
                {selectedStory.program_name}
              </span>
            )}
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '1.5rem' }}>
          {successStories.map(story => (
            ...
            >
              ...
              <p style={{ color: '#718096', fontSize: '0.95rem', marginBottom: '0.75rem' }}>
                {story.resident_name}
              </p>
              ...
              {story.program_name && (
                <span style={{ background: '#e6f2ff', color: '#667eea', padding: '0.4rem 0.8rem', borderRadius: '20px', fontSize: '0.85rem', fontWeight: '500' }}>
                  {story.program_name}
                </span>
              )}
            </div>
  ...
  const renderEvents = () => (
    <div>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: '#718096' }}>Join us for workshops, celebrations, and community gatherings</p>
      </div>
      ...
      {selectedEvent ? (
        <div style={{ background: 'white', padding: '2rem', borderRadius: '12px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '2rem', padding: '1.5rem', background: '#f7fafc', borderRadius: '8px' }}>
            <div>
              <p style={{ fontSize: '0.85rem', color: '#718096', marginBottom: '0.25rem' }}>Date & Time</p>
              <p style={{ fontSize: '1.1rem', fontWeight: '600' }}>📅 {formatDate(selectedEvent.event_date)}</p>
            </div>
            <div>
              <p style={{ fontSize: '0.85rem', color: '#718096', marginBottom: '0.25rem' }}>Location</p>
              <p style={{ fontSize: '1.1rem', fontWeight: '600' }}>📍 {selectedEvent.location}</p>
            </div>
            <div>
              <p style={{ fontSize: '0.85rem', color: '#718096', marginBottom: '0.25rem' }}>Event Type</p>
  ...
  const renderTestimonials = () => (
    <div>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: '#718096' }}>Hear what our community members have to say</p>
      </div>
        ...
        {testimonials.map(testimonial => (
          <div
            ...
            style={{
              ...
              borderRadius: '12px',
              boxShadow: '0 2px 8px rgba(0,0,0,0.1)',
              border: testimonial.is_featured ? '2px solid #fbbf24' : '1px solid #e2e8f0'
            }}
          >
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <p style={{ fontSize: '1.05rem', lineHeight: '1.7', color: '#4a5568', fontStyle: 'italic' }}>
            ...
            <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '1rem' }}>
              ...
              {testimonial.program_name && (
                <span style={{ background: '#e6f2ff', color: '#667eea', padding: '0.3rem 0.6rem', borderRadius: '20px', fontSize: '0.8rem', fontWeight: '500' }}>
                  {testimonial.program_name}
                </span>
              )}
            </div>
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              fontSize: '1.05rem',
              fontWeight: '600',
              color: activeSection === section.id ? '#667eea' : '#718096',
